...
- **The self-scraping tier is retired (Phase 36, ADR-139).** There is no `universal_ai` adapter, no `vendor_quirks.yaml` registry, and no `sync-prompt` generator anymore — recall is Serper + eBay only (`run_v2`). Do not reintroduce per-vendor bot-wall machinery (`use_scrappey`, `alterlab_options`, `force_detail_backup`, `search_url_template`); if a recall gap appears, address it in the Serper/eBay layer. (The dead `worker/.../onboarding/prompts/onboard_v1.txt` and the frozen `web/lib/onboard/promptText.ts` + `vendor-quirks-data.ts` are legacy v1-onboarder artifacts kept only for the v1 validation-guard tests; they are NOT regenerated.)

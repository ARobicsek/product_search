...
_FIXTURE_DIR = Path(__file__).parent.parent.parent.parent / "tests" / "fixtures" / "memstore"

def _fixture_dir() -> Path:
    if _FIXTURE_DIR.is_dir():
        return _FIXTURE_DIR
    cwd_path = Path.cwd() / "tests" / "fixtures" / "memstore"
    if cwd_path.is_dir():
        return cwd_path
    for parent in Path.cwd().parents:
        candidate = parent / "worker" / "tests" / "fixtures" / "memstore"
        if candidate.is_dir():
            return candidate
    return _FIXTURE_DIR
...
def _parse_html(html: str) -> list[Listing]:
    ...
    for item in tree.css(".s-item__wrapper"):
        ...
        listings.append(Listing(
            ...
            seller_name="mem-store",
            seller_rating_pct=None,
            seller_feedback_count=None,
            ship_from_country="US",
            # Explicitly applying the generic-MB-modules flag logic
            flags=["generic_brand"] if "Generic Brand" in title else []
        ))
...
def _fetch_live(query: AdapterQuery) -> list[Listing]:
    ...
    url = f"https://www.ebay.com/sch/{query.seller_id or 'mem-store'}/m.html"

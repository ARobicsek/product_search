...
> **Status (2026-05-27):** ADR-105 (registry-driven search URLs) + ADR-109 (honest per-source diagnostics) + ADR-106 (parser-gap recovery via the embedded-state `__NEXT_DATA__` tier) + ADR-108 (onboard prompt diet, `onboard_v1.txt` cut 44% by words) are **DONE/ACCEPTED**. **Remaining, in order:** (1) **ADR-110 (PROPOSED)** — onboarder turn must never strand the user (per-turn budget + force-finalize + graceful recovery + run-time "0 finds → re-probe" guidance); surfaced when the ADR-108 live verification froze on a Walmart-504 retry loop. (2) **Resume the ADR-108 3× DJI MCP verification** (blocked by that freeze). (3) **ADR-107** — generalize auto-Scrappey; needs a small live AlterLab+Scrappey spend (~$1–3). See ADR-110 body in DECISIONS.md for the full plan.
...
## Phase 38 — Amazon recall via DataForSEO (PAYG, structured) (PROPOSED 2026-06-01 — ADR-141; Step 1 spike DONE → GO)

> Activates the REBUILD_PLAN §0/§10 *"Future: Amazon adapter"* extension point (the Phases 30–36 rebuild arc is complete; it was tracked in [REBUILD_PLAN.md](REBUILD_PLAN.md), not here). **Read [ADR-141](DECISIONS.md) first** — it records the decision, the verified PAYG cost analysis, and *why not Scrappey*. Don't re-debate those points. (Numbered 38, not 37: Phase 37 is "Smart Listing Cards," claimed by a parallel session — see `docs/NEXT_SESSION.md`.)
...
**38a — worker recall path** (Steps 1–5, 8): de-risk + adapter + profile + wire-in + tests. **38b — onboarder + deploy + live verify** (Steps 6–7, 9): onboarder toggle, workflow secret mapping, live run.

### Tasks

**Step 1 — Recall sanity-check. ✅ DONE 2026-06-01 — decisive GO** (`worker/scripts/amazon_spike.py` + 5 raw fixtures in `worker/tests/fixtures/amazon/` committed). Fired 5 queries at the **live/advanced** endpoint (depth 100): DJI Neo 2 Motion Fly More Combo **124** products (price 95% / asin 100% / url 100% / rating 90%); Supermicro H14SSL-N **22** (86/100/100/55); lululemon Never Lost Keychain **114** (100/100/100/100); DDR5 RDIMM ECC 64GB **113** (99/100/100/48); The Week magazine subscription **41** (price **22%** — mostly back-issues, **confirms Amazon-off-for-subscriptions** default). The exact target ranked at/near the top in all four physical-goods queries — recall Serper structurally can't give. Cost **$0.0033/query** (live, depth-100; Standard queue + lower depth will be cheaper). **Build gotchas the spike surfaced (don't rediscover):** (a) DataForSEO Amazon requires `location_name: "United States"` **+ `language_name: "English (United States)"` (or `language_code: "en_US"`) — NOT `"English"`** (else task-level `40501 Invalid Field: 'language_name'`); (b) top-level `status_code: 20000` can mask a **task-level** error in `tasks[0].status_code` — the adapter MUST check the task status, not just the envelope; (c) new accounts return `40104 verify your account`, and verification propagates with eventual consistency (intermittent 403s for a few minutes right after verifying) — treat non-20000 task status as `AmazonAPIError`. Confirmed mapping fields: `data_asin`/`title`/`url` (real `/dp/`)/`price_from`/`price_to`/`currency`/`rating{value,votes_count,rating_max}`/`image_url` (real CDN URL, no base64)/`is_amazon_choice`/`is_best_seller`/`type` (`amazon_serp` organic vs `amazon_paid` sponsored)/`delivery_info`/`bought_past_month`. *(Original brief: fire ~5 representative queries; GO if recall + price completeness beat "nothing"; NO-GO ⇒ provider-agnostic fallback to RapidAPI/Rainforest.)*

**Step 2 — `adapters/amazon.py`. ✅ DONE 2026-06-01 (38a).** (mirror `adapters/serper.py` structurally). `fetch(query, *, fixture_path=None) -> list[Listing]`; `WORKER_USE_FIXTURES` + single-file fixture mode; live = DataForSEO **Basic auth** (`DATAFORSEO_LOGIN`/`DATAFORSEO_PASSWORD` from env or `worker/.env`), default **Standard queue** (`task_post` → poll `task_get`/`tasks_ready`), `priority` switch for Live. Map JSON → `Listing`: `source="amazon_dataforseo"` (adapter id, **not** merchant); `seller_name` = merchant/"Amazon"; `url`==`buy_url` = the **real `/dp/<ASIN>` Amazon URL** (unlike Serper, Amazon yields a direct merchant link — distinct-field hygiene still holds); `attrs["asin"]`; `rating`/`rating_count`/`image_url`; `condition` from data else `""` (honest-degraded, like Serper — never `"unknown"`, which `reject_condition_in` would reject 100%); `quantity_available=None` (search-only). Exceptions `AmazonAuthError`/`AmazonAPIError` mirroring Serper. **Write the provider behind a thin internal seam** (`_PROVIDER` config) so RapidAPI/Rainforest can swap without touching the pipeline. Dedup by ASIN→url.

**Step 3 — Profile schema. ✅ DONE 2026-06-01 (38a).** Add `AmazonSource(enabled: bool = False, depth: int = 48, priority: Literal["standard","live"] = "standard")` to `SourcesV2` in `profile_v2.py`; **off by default**. Test in `test_profile_v2.py`. *(Shipped exactly as specced; +4 schema tests.)*

**Step 4 — Wire into the run spine. ✅ DONE 2026-06-01 (38a).** In `run_v2._default_recall`, add an `if profile.sources.amazon.enabled:` block with its own `amazon_error` flag in `RecallOutcome` (per-source independent failure, like eBay); thread `amazon_error` through `run_v2_pipeline` → `classify_run_outcome`; add the additive **`amazon_unavailable`** note to `run_outcome.py` (mirror `ebay_unavailable`) + REBUILD_PLAN §8 / honest-error-taxonomy. Ship-from gate is a no-op for US Amazon (like Serper). `_dedup_union` already collapses cross-source URL collisions.

**Step 5 — Run-cost panel. ✅ DONE 2026-06-01 (38a).** Record Amazon spend as a flat per-query API cost (recall is a flat-fee call; surface it in `run_calls`/the cost table so per-run Amazon cost is visible — honest cost accounting). Consider adding Serper's recall cost at the same time (currently only `ai_filter` is panelled). *(Shipped: the adapter sums the real DataForSEO `cost` field into `amazon.LAST_RUN_COST_USD`; `run_v2` appends an `amazon_recall` `run_call` with an explicit `cost_usd`, and `_build_run_cost` now uses an explicit `cost_usd` directly instead of token-estimating. **Serper's recall cost was NOT added** — the Serper adapter exposes no cost field and hardcoding a per-call estimate would fabricate; left as a follow-up if Serper starts returning credit usage.)*
...
**Step 8 — Tests + fixtures** (offline). **✅ DONE 2026-06-01 (38a).** Committed DataForSEO Amazon fixture(s) under `worker/tests/fixtures/amazon/`; `test_amazon.py` (mapping, ASIN dedup, auth error, fixture mode — mirror `test_serper.py`/`test_ebay.py`); `AmazonSource` test; run_v2 wire-in test (`amazon_error` → `amazon_unavailable`); cross-source dedup test. *(Shipped: `test_amazon.py` 11 tests over the 5 Step-1 fixtures incl. the task-level-status gotcha + the missing-creds auth path via a monkeypatchable `_env_files()`; +4 schema, +1 run-outcome, +4 run_v2 wire-in, +1 cost-panel. Worker 365/365.)*
...
## Phase 42 — Local qwen-coder filter backend + polite shared-server coordination + qwen-coder-vs-Haiku live A/B (ADR-147)

> **STATUS 2026-06-28 — Tasks 1, 2, 4 DONE; coordination live-validated; Task 3 (live multi-slug A/B) DEFERRED to box availability.** Shipped the pluggable backend (`config.FilterBackendConfig` + `AI_FILTER_BACKEND`; `local` provider in the LLM seam; `ai_filter` resolves backend + Haiku stays default/fallback) and the polite coordinator (`llm/local_box.py`). **The open "activity/idle signal" question is RESOLVED:** `/api/metrics` gives per-model last-*completed* timestamps (logs on completion only → a long in-flight request is invisible), so the coordinator pairs it with a live active-now guard (`/api/events` SSE `inflight`, GPU-load fallback) — confirmed necessary by a LIVE case where a resident model showed a 3.8h-stale metric while actually serving (inflight=1, 229W). +20 worker tests (419 total), ruff+mypy clean; live-validated against the busy box (waited politely → Haiku; fixed a Windows-cp1252 log crash). **Task 3 not done:** the box was in continuous use by another session all session; loading qwen-coder is a swap the new etiquette forbids while someone's active. Run `scripts/serper_filter_bakeoff.py --only local-qwen-coder` vs `--only haiku-4.5` (4 slugs: ddr5/dji/the-week/the-netanyahus) when the box is free. Details in ADR-147.

...
`web/lib/dispatch.ts` + `web/app/page.tsx` + `web/app/CardRunStatus.tsx`: per-card `status: running|waiting|idle`; amber **"Waiting to run"** when scheduled-but-idle; green **"Running since &lt;begin time&gt;"** while active (the active run's `run_started_at`, never the stale last-run instant). tsc + lint clean. Verified live at 390px: mobile layout intact, `idle` state correct (9255 shows the fresh post-#26 time + recovered $2117.44 summary), zero console errors, "No reports yet" null-path preserved. **Not visually reproduced** (honest gap): `waiting` / `running` — origin/main had no scheduled product at commit time and the page reads live from origin/main; logic-verified only.

### Next session / next in THIS session — Task 1 (the original ask, NOT yet started)

The session's **first** item is still open: the alerts → push-subscribe UX is confusing; the user wants the per-product PWA-only "Enable/Disable Alerts" control replaced by **a single bell on the home screen** (illuminated bell = subscribed, greyed/crossed-out bell = not subscribed). Backend constraints already diagnosed (see the ADR-053 status block "Noticed but deferred — alerts UX confusion"): the subscription is **device-wide** (single global Redis set, `productSlug` unused), and the control currently `return null` outside the installed PWA. Per memory `feedback_interview_before_ux_work`: run the structured trade-off interview (visibility on desktop vs PWA-only; scope-clarity wording; whether email-on-alert is in scope) **before** coding. Phase 18 — Polish + second-product proof remains queued after.

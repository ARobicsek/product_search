...
- **Queued next:** **Phase 18 — Polish + second-product proof**, OR the highest-priority defect from Phase 26 (Defect 1: onboarder drops detail-preferred URLs before save gate; Defect 2: ADR-084 callout swallows per-source HTTPError rows due to host-aggregated `passed`). User to pick.
...
1. **Phase 26 Defect 1 — make ADR-079 hard to bypass.** Add a prompt rule that says "if a detail-preferred host's probe fails, KEEP the URL in `sources` with the probe result as a `note`; NEVER drop it to `sources_pending` and NEVER emit a URL-less placeholder note", and add a deterministic save-time guard that warns when a `PREFER_DETAIL_HOSTS ∪ FORCE_DETAIL_BACKUP_HOSTS` host has zero `universal_ai_search` sources for it in the draft. Reuse the existing ADR-067/074/080 guard pattern in `web/lib/onboard/`. Cheap, no live runs needed (use Phase 26's saved profile fixtures as the regression input).
2. **Phase 26 Defect 2 — fix ADR-084 callout for per-source HTTPError rows.** Audit how `source_stats[i].passed` is built in `cli.py` (search loop); appears to be host-aggregated or run-aggregated. Add a regression test in `test_cli.py` mirroring the live xm5 shape (same host as 4 rows, one OK + 3 erroring) — assert each erroring row's per-source `passed` is `0` and lands in the callout. The classifier itself is fine.
3. **Phase 26 Defect 3 — re-probe microcenter and update the registry.** Run `cli probe-url --render` against 3 distinct microcenter URLs at the current defaults; if ≥2 of 3 succeed, downgrade `known_failure: blocker` → `warning` (or remove) and regenerate `vendor-quirks-data.ts`/`promptText.ts` via `sync-prompt.js`. Tiny scope, immediately ships value.
4. **Phase 18 — Polish & second-product proof** (the originally-queued next phase).

**Other standing candidates** (only if none of the above is picked):

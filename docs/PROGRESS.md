...
**Inter-phase — alert push delivery was NEVER wired (ADR-057) + new `while_below` every-run mode (ADR-058). DONE in-repo 2026-05-18; ADR-057 needs an out-of-repo runbook.** User: alert + hourly schedule on, bell purple, price < $2700, "not getting alerts — nothing at all, ever." Evidence-based root cause (ADR-057): the `is_below` alert *did* fire server-side every qualifying run (proven via `alerts_state.json` arming history) but `notify.py` early-returns without POSTing when `WEB_URL`/`PUSH_NOTIFY_SECRET` are unset — and **neither search workflow ever set them** (only API keys). `cli.py` disarms regardless of notify success, so it looked "fired" while zero pushes were ever sent — push has **never worked once** in CI. Fix: added both vars to `search-scheduled.yml` + `search-on-demand.yml` step `env:` and to `.env.example`. **Inert until the user sets GH Actions secrets `WEB_URL`+`PUSH_NOTIFY_SECRET` and confirms Vercel has matching `PUSH_NOTIFY_SECRET` + VAPID keys + Upstash, then redeploys** (Claude cannot — flagged). Separately, per a trade-off interview the user asked for explicit "once vs every run while below" UI choice → ADR-058 adds stateless `mode: while_below` (fires every run while cheapest < threshold; zero-listing run = skip that run, ship-simple); editor now offers exactly `is_below` ("once") + `while_below` ("every run"), `drops_below` retired from the picker (legacy-only). Worker 255 passed (4 new); ruff+mypy clean; web tsc+eslint clean. **Next: user completes the ADR-057 runbook, then we push + verify the armed `amd-epyc-9255` rule delivers on the next run.** See ADR-057, ADR-058 + the dated status block below.
...
**Inter-phase — delete-product UX: touch-reachable trigger + portaled modal + post-delete reload. DONE in-repo 2026-05-18 (ADR-063).** User reported delete impossible on the iPhone PWA, and on desktop the modal text overlapped card text + the list didn't refresh after delete. Three independent fixes: (1) removed the `opacity-0 group-hover/focus-within` gate on the trash wrapper in `web/app/page.tsx` (no `:hover` on touch + stretched card link made it unreachable — CLAUDE.md mobile rule); (2) `DeleteProductModal` now `createPortal`s the overlay to `document.body` — the "overlap" was a stacking bug (overlay's `z-50` trapped inside the card's `flex … relative z-10` context; later cards' `relative z-10` summaries painted over the *opaque* panel — proven via DevTools computed styles, not a transparency bug); (3) success now `window.location.reload()`s (router.refresh insufficient per `project_nextjs_cache_runnow`). Verified with chrome-devtools at 390px: trash icons on every card, modal opaque/no bleed-through, portal parent == body, Cancel closes; tsc+eslint clean. **NOT end-to-end verified:** actual DELETE→reload (local `WEB_SHARED_SECRET` unset → route 500s; a real delete commits to origin/main = destructive). See ADR-063.

## Status as of 2026-05-18 (inter-phase — ADR-063: delete-product touch-reachable + portaled modal + post-delete reload)

### What shipped

- **ADR-063** — three independent delete-product defects the user hit in prod: (1) **iPhone PWA: delete impossible** — `web/app/page.tsx` trash wrapper was `opacity-0 group-hover:opacity-100 focus-within:opacity-100`; touch has no `:hover` and the card's stretched `before:absolute inset-0` link swallows taps, so the icon was invisible+unfocusable. Removed the gate (icon always shown; already subtle `text-gray-400`). (2) **Desktop modal "overlap"** — root cause was *stacking*, not transparency (DevTools showed panel `bg` opaque `lab(8%)`, `opacity:1`): the `fixed z-50` overlay rendered inside the card's `flex items-center gap-2 relative z-10` stacking context, so later cards' `relative z-10` summaries painted over it. `DeleteProductModal` now `createPortal`s the overlay to `document.body`. (3) **No refresh after delete** — modal only did `setIsOpen(false)`; now `window.location.reload()` on success (keeps `isDeleting` spinner through nav; `router.refresh()` documented-insufficient — memory `project_nextjs_cache_runnow`).

### Verification

chrome-devtools at **390px** (iPhone-ish), against the user's running `:3000` dev server (Next 16 single-instance lock prevents a second dev server from the same dir — could not run an isolated instance): trash icons present+reachable on every card; modal opens **opaque, zero bleed-through**; `overlay.parentElement === document.body` confirmed (portal live after HMR); Cancel closes. `npx tsc --noEmit` + `npx eslint` clean on `DeleteProductModal.tsx`+`page.tsx`. **Open gap:** the real DELETE→reload path is unverified locally — `WEB_SHARED_SECRET` is unset in dev (route returns 500) and a genuine delete commits to `origin/main` (destructive, not run). The reload is a 1-line change on documented precedent; confirm on the deployed app next session.

### Next session — unchanged carry-over

ADR-063 is in-repo + committed; spot-check the real delete→reload on the deployed app when convenient. Otherwise the prior queue stands: **REVIEW PROD TEST RESULTS** for the deployed Schedule&Alerts editor (ADR-059/060/061) — esp. the never-Claude-verified mobile popover layout — then **Phase 18 — Polish + second-product proof**. Still-open carry-overs unchanged: ADR-040 auto-demote impl; ADR-053 deferred #1–#3; scheduled tick #25 06:11Z failure; no in-app signal for silent external-trigger death; email-on-alert (own ADR + sign-off).

### Noticed but deferred

`web/lib/onboard/promptText.ts` is still modified-but-uncommitted (dev-server `[sync-prompt]` auto-regen, carried forward, intentionally not part of ADR-063). Untracked `scratch/aufschnitt.html` left as-is. The Next 16 same-dir single-dev-server lock means future UI verification must reuse the user's running server (or stop it first) — noted so the next session doesn't waste time on a parallel instance.

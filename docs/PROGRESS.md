...
- **NEW (2026-05-24 eval, user-directed):** 3-product onboard+run study (AirPods Pro 2 USB-C, MX Master 3S, KitchenAid KSM150PS) confirmed **precision is excellent, recall is the bottleneck**. Top actionable recall fixes: (1) **retry AlterLab on 5xx before the curl_cffi fallback** — a 504 currently drops to a no-JS/no-proxy tier that every bot-walled retailer blocks, silently zeroing recall (the escalation ladder only fires on weak *200* bodies); (2) **global per-run time/cost circuit breaker** — a 7-source run under degraded AlterLab took >28 min; (3) onboarder must **enforce `prefer_page_type:detail`/`force_detail_backup` regardless of a transient probe failure** (it downgraded B&H detail→search → 0 recall) and treat probe as advisory not a gate. Precision fixes: stop the onboarder emitting substring-fragile `title_excludes` (`"MX Master 3"` ⊂ `"MX Master 3S"`; `"bowl"` false-rejected a real "…with Copper Bowl" mixer). Full detail in memory `project_recall_precision_eval_2026_05_24.md`. NB: 3 throwaway test slugs were left LIVE on origin/main with daily cron (cleanup classifier-blocked) — delete `products/{airpods-pro-2-usb-c,logitech-mx-master-3s,kitchenaid-ksm150ps-mixer}` + their `reports/` when convenient.

...
- **ADR-121** — Save-time probe modal UX clarity (PROPOSED, 2026-05-28, Phase 29). Probe modal makes "Continue probing" feel infinite because the DETAIL-URL BACKFILL section grows visually with each retry while the header just says "Probe attempt N." Decision: per-host roll-up at top, planned-cap explicit in header, re-label BACKFILL section, after 3 fruitless retries the Continue button becomes "Stop and save what we have." Brief: see [SESSION_C_BRIEF.md](SESSION_C_BRIEF.md) Defect F.
...
## ADR-121 — Save-time probe modal UX clarity

**Status**: PROPOSED — diagnosed 2026-05-28 in the ADR-114/115 live test.

**Date**: 2026-05-28

**Context.** User's verbatim feedback after the live ADR-115 probe modal test: *"it kept adding more searches for no reason I could see every time I asked it spend more time probing — I wondered if that cycle would just last forever so I eventually just proceeded with what I have."* The screenshots show the `DETAIL-URL BACKFILL` section growing 1→4 target.com candidates across "Probe attempt 2" → "Probe attempt 4" while the modal header only says "Probe attempt N." Each click of *Continue probing* re-plans backfill candidates (because more search-page probes have completed in the meantime), so the visual impression is "the system keeps adding work, this will never end." In reality the planned set is bounded per host (`MAX_BACKFILL_PROBES_PER_HOST = 3` per ADR-076) — the modal just doesn't show that bound.

**Decision.** Make the bound visible and add an explicit out:
1. Header: replace "Probe attempt N" with "Probing N vendors — X of Y URLs done, Z planned."
2. Per-host roll-up at top: "amazon.com: 1/2 ✓, microcenter.com: 0/1 ⏳" so the user sees pending work at a glance.
3. Re-label "DETAIL-URL BACKFILL" → "Looking for product detail pages on hosts that only have a search URL" + a one-line `<details>` explainer.
4. After 3 successive "Continue probing" clicks with no new completion progress, the Continue button is replaced by a "Stop and save what we have" CTA (still subject to ADR-111 gates — but at least the user knows the loop is bounded).
5. New SSE event `plan_summary` emitted at the start of each pass with `{ totalUrls, plannedBackfill, byHost }` so the modal has the data without inferring.

**Consequence.** The user sees an explicit ceiling on planned work, can tell whether re-probing is actually making progress, and has an explicit "I'm done waiting" path. No worker changes; web-only. ADR-076's per-host cap stays — just made user-visible. Cost: zero extra probes (the cap was always there; just hidden).

Full files-to-touch + acceptance criteria: [SESSION_C_BRIEF.md](SESSION_C_BRIEF.md) Defect F.
...
Full files-to-touch + acceptance criteria: [SESSION_C_BRIEF.md](SESSION_C_BRIEF.md) Defect A.

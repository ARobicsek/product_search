...
- **ADR-152** — A dead filter backend is its own run outcome, not `all_filtered` (ACCEPTED, 2026-08-06). **Context:** while diagnosing a wave of red scheduled runs (which turned out to be an unrelated GitHub-wide Actions outage — see PROGRESS), the 2026-08-06 14:16 run on `ddr5-32gb-ecc-rdimm-server-ram` was found zeroed: the owner's local GPU box was down, so the ADR-147 local-only filter chain (`qwen-coder` → `qwen3.6-27b-mtp`, no Haiku per `LOCAL_LLM_ALLOW_HAIKU_FALLBACK=0`) exhausted. `filter.jsonl` held exactly one sentinel row — `ai_filter exception: LLMError('LOCAL connection error: Request timed out.') (batch 1)` — and `run_cost` showed `provider=local model=qwen3.6-27b-mtp` with **0 input / 0 output tokens**. But the report told the user *"none matched your spec — your match/filters may be too strict, or the query is mis-scoped."* **The problem:** `ai_filter` returns `[]` both when the model honestly rejects everything and when every model in the chain failed, so `classify_run_outcome` (which sees only counts) cannot tell them apart and falls through to `all_filtered`. The two are identical from the outside (offers recalled, 0 survivors) but have **opposite fixes** — one is ours and self-heals next run, the other tells the user to loosen their spec. Since ADR-149 made a local box the sole prod backend, every box outage now silently masquerades as a bad profile, and the owner is invited to "fix" a profile that was never broken. **Decision:** make the distinction explicit rather than inferring it. `ai_filter` gains a module-level `LAST_RUN_FAILED` flag — set only at the chain-exhaustion site, reset at the top of `ai_filter` and in `reset_last_run()` (so a multi-product scheduler tick can't carry one product's outage into the next product's outcome, the same staleness class ADR-148 fixed for cost). `run_v2_pipeline` threads it into a new `filter_error` argument, and `RunOutcomeClass` gains **`FILTER_UNAVAILABLE`**, ranked *above* `ALL_FILTERED` and *below* `NO_RECALL` (a genuine 0-offer recall is still the more fundamental fact, and the flag is ignored when survivors exist — a mid-run failure that later batches recovered from is an `ok` run). Message names it as ours: *"Found offers but the relevance filter couldn't run… This is a backend problem, not your profile — the next scheduled run retries automatically."* **Rejected:** inferring the outage from the `(filter call failed)` sentinel row in `LAST_RUN_LOG` — that re-derives a fact the filter already knows by pattern-matching a log string, and would break the moment the message is reworded. **Verified:** worker **445** (+7), ruff(src+tests)+mypy clean. No web change needed — `ResultView.OutcomePanel` renders `outcome.class`/`message` generically and `result-types.ts` types `class` as `string`, so the new class surfaces with the correct non-ok pill automatically. **Consequence:** the general rule — when a component returns an empty result, an outage and a negative verdict must be distinguishable at the seam; the classifier must never have to guess which one it got. **Status: ACCEPTED.**

- **ADR-151** — Viewing a profile must be free; the onboarder is the only paid path (ACCEPTED, 2026-07-03). **Context:** the only way to inspect a slug's `profile.yaml` from the app was the product page's "Edit Profile" link → `/onboard?edit=<slug>`, which mounts `OnboardChat`; that component fires an LLM turn *on mount* (the edit-mode kickoff hands the whole YAML to the model and asks it to acknowledge + ask what to change — [OnboardChat.tsx](../web/app/onboard/OnboardChat.tsx) `useEffect`→`runTurn([kickoffMessage])`). So merely *looking* at a profile spent tokens — friction against the owner's cost-sensitivity (PAYG-only, per-call cost-conscious). **Decision:** add a read-only viewer that never touches the LLM. The product page already fetches `profileYaml` server-side (for the Schedule & Alerts editor), so a **"Profile" button** opens a **client-only modal** rendered entirely from that in-hand string — no network call, no API route, no LLM, $0. A new pure `web/lib/profile-summary.ts` (`summarizeProfile`) parses the v2 YAML into friendly sections (tracking/target, queries, matching, filters, sources w/ enabled state, flags, vendor lists, display); schedule + alerts reuse the existing `humanizeSchedule`/`describeRule` humanizers; a progressive-disclosure "View raw YAML" shows the exact file. Both "Edit Profile" links in `page.tsx` are replaced by the viewer; the one token-spending path (`Edit with the assistant →`, links to `/onboard?edit=`) lives *inside* the modal with an explicit "Viewing is free. Editing uses tokens." note — so the cheap action is the default and the paid one is deliberate (good-UI principle: make the common/cheap action the default, the expensive one explicit). **Verified:** web guards **79/79** (+9 in `check-profile-summary.test.mjs`, wired into `test:guards`), tsc/eslint/`next build` clean; Playwright @375px on `supermicro-h14ssl-n` + `the-week-us-one-year-subscription` — modal renders, raw toggle works, edit link correct, disabled sources shown muted, 0 console errors, **0 API calls on open**. No worker changes. **Consequence:** the general rule going forward — a read/inspect surface must never mount the onboarder; anything that fires an LLM turn belongs behind a deliberate, labeled affordance. **Status: ACCEPTED.**
- **ADR-150** — Alias match is a GATE/SIGNAL for the relevance LLM, never an auto-pass (ACCEPTED, 2026-06-28). **Supersedes the ADR-145/146 deterministic alias auto-pass and reworks ADR-148's `variant_strict`-skips-LLM.** **Context:** the 2026-06-28 prod runs of `focal-clear-mg` (headphones) and `supermicro-h14ssl-n` (motherboard) had terrible precision — reports dominated by accessories. Root cause (diagnosed from the committed `reports/**` filter logs): the ADR-145/146 deterministic alias pre-pass (`partition_by_exact_alias`; `title_has_exact_alias` = plain case-insensitive **substring** match) AUTO-PASSED — bypassing the LLM — every listing whose title contained a `match.aliases` string. Safe for a globally-unique RAM MPN, but short model-name aliases ("Clear MG", "H14SSL-N") appear inside accessory/compat titles ("ear pads **for** Clear MG", "32GB memory **for** H14SSL-N", "I/O shield ... H14SSL-N") and inside superstring SKUs ("H14SSL-N" ⊂ "H14SSL-N**T**"). Evidence: focal 120 survivors = **110 alias auto-passes** (never seen by the LLM) + 10 LLM; supermicro 38/38 auto-passed, LLM passed 0/192. A smarter model could not help — the LLM was never consulted on those listings. **Decision (owner-directed):** the alias match is no longer an auto-pass; the LLM ALWAYS makes the final relevance call. (1) **Non-strict (`variant_strict:false`):** ALL deterministically-passed listings go to the LLM; the alias match rides along per-listing as a `model_name_in_title` boolean signal (strong "this IS the product" evidence that does NOT override the accessory-rejection rule). (2) **Strict (`variant_strict:true`+aliases):** the alias match GATES the candidate set (non-alias listings dropped — the user asked for ONLY the exact part), then the LLM still judges the gated set so it can strip accessories/bundles that merely name the model. (3) `title_has_exact_alias` upgraded from substring to **token-boundary** matching (adjacent char non-alphanumeric or string edge) so "H14SSL-N" no longer matches "H14SSL-NT" while still matching "MBD-H14SSL-N-O". **Wiring:** `run_v2` step 4 rewritten (gate/pass-all, always-LLM); `to_filter_profile` populates the existing `Profile.match_aliases` (ADR-099 field, distinctiveness-validated; new `distinctive_aliases` helper = shared source of truth for gate + signal); `ai_filter` adds the per-listing `model_name_in_title` field + a prompt rule (only when aliases present); removed the now-dead `partition_by_exact_alias` / `title_states_excluded_condition` / `_CONDITION_CUES`. **Verified offline against the REAL 2026-06-28 recalls on the live box (qwen-coder, no re-scrape):** focal 120 → **28 survivors, all genuine Clear MG** (0 accessories; old wrong-model leaks — original Clear / Clear Professional / Bathy MG — also gone); supermicro 38 → **14 survivors, all real H14SSL-N boards** (0 I/O shields, 0 H14SSL-NT, 0 RAM kits, 0 CPU bundles). Worker 438 (−5 partition/condition tests, +8 boundary/signal/gate tests), ruff+mypy clean; no web changes. **Tradeoff (accepted):** exact parts now pass back through a non-deterministic LLM (the ADR-145 concern), but the `model_name_in_title` signal + fixed prompt strongly bias toward keeping them, and the strict gate prevents the LLM from ADDING equivalents (the ADR-148 concern) since it only ever sees the gated set. **Pushed to `origin/main` (commit `8073b8c`) and LIVE-VERIFIED in prod 2026-06-28** — owner re-ran both `focal-clear-mg` + `supermicro-h14ssl-n` on-demand and confirmed the reports are now accessory-free. **Status: ACCEPTED.**
- **ADR-149** — Local-filter prod cutover: **Option B (cloud runner + Tailscale to the box), NOT Option A (self-hosted runner on the box)** (ACCEPTED, 2026-06-28). **Supersedes the ADR-145/146/147 "deferred reachability" + the PROGRESS NEXT-SESSION Option-A plan.** **Context:** the local qwen-coder ai_filter backend is prod-ready (`AI_FILTER_BACKEND`, `LOCAL_LLM_*`, no-Haiku flag, $0 pricing, failure push — ADR-147/148); the only open item was GH-Actions→home-box reachability. PROGRESS had penciled in **Option A** (install a self-hosted GH Actions runner on the box). **Why Option A was rejected (owner call):** (1) it dirties the owner's clean-Ubuntu box with a runner + git + Python + uv; (2) the scheduled workflow ticks every ~15 min and the due-ness decision runs *inside* the job ([cli.py:1062](../worker/src/product_search/cli.py#L1062)), so a self-hosted-everything setup needs the box online 24/7 just to service empty no-op ticks → the box becomes a continuous SPOF for *all* prod scheduling. **Decision (Option B):** keep `runs-on: ubuntu-latest`; the box stays a pure network AI service (llama-swap @ `100.68.68.101:8080`, already on the tailnet — **nothing new installed on it**); the ephemeral cloud runner joins the tailnet per-run via `tailscale/github-action@v2` (OAuth client, tag `tag:ci` — OAuth over a raw authkey to avoid the 90-day-expiry footgun) and calls the box only when a search actually fires. So the box only needs to be up when a run is genuinely due / on-demand, not every 15 min. **Wiring (this session, both `search-scheduled.yml` + `search-on-demand.yml`):** added a Tailscale connect step gated `if: vars.AI_FILTER_BACKEND == 'local'` + the `AI_FILTER_BACKEND`/`LOCAL_LLM_*` env from repo Variables (`|| default` fallbacks). **Safe-by-construction:** with the `AI_FILTER_BACKEND` Variable unset the Tailscale step is skipped and the worker stays on Haiku ([config.py:117](../worker/src/product_search/config.py#L117)) — merging is a true no-op for current prod. **Owner prerequisites (verified NOT yet done: 0 runners, 0 Variables):** (a) create a Tailscale OAuth client (scope `auth_keys`, tag `tag:ci`) → add repo secrets `TS_OAUTH_CLIENT_ID` + `TS_OAUTH_SECRET`; (b) ACL: allow `tag:ci` → `100.68.68.101:8080`; (c) set repo Variable `AI_FILTER_BACKEND=local` (optionally override `LOCAL_LLM_*`); (d) **leave `LOCAL_LLM_ALLOW_HAIKU_FALLBACK` unset (Haiku-on-unreachable) during bring-up; set it to `0` only after the box proves reliable** to enforce the no-Haiku/$0 rule. Then trigger an on-demand run on `ddr5-32gb-ecc-rdimm-server-ram` to confirm filter-on-qwen-coder + ~$0 cost + outcome ok. **Tradeoff:** prod now depends on Tailscale being up + the box being reachable when a run fires (vs. a paid Haiku call); the no-Haiku flag turns a box outage into a zeroed run + push alert (acceptable per ADR-147). **Verified LIVE in prod 2026-06-28** (run `28332773666`, on-demand `focal-clear-mg`, non-strict, 3 sources): every step green incl. **Connect to Tailscale**; the GH-hosted runner reached the home box over the tailnet and the filter ran on **provider=local / model=qwen-coder** (91,848 in / 11,561 out tok, **$0.00**); total run cost **$0.004** (Amazon recall only), recall 336 → 121 survivors, **outcome ok**. Owner used the tailnet's default allow-all ACL (no per-host rule), so only the `tag:ci` tagOwner was needed. **No-Haiku ENFORCED 2026-06-28** (owner-requested, right after the verify run): Variable `LOCAL_LLM_ALLOW_HAIKU_FALLBACK=0` set → local-only filter chain even when the box is busy/unreachable; a box outage → zeroed run + push alert, never a paid Haiku call. **Card UI:** the homepage product card now surfaces the filter model (`· filter: qwen-coder` / `· filter: Haiku 4.5`) from the run_cost `ai_filter` step (absent when no LLM ran). **Post-run box release (owner-requested):** after the filter, the worker politely unloads our local model from the shared box (`local_box.unload_after_use` → llama-swap `GET /unload`) — ONLY when our model is the sole resident AND the box is known-idle (no in-flight request), so it never evicts a model another session swapped in nor interrupts an in-flight request; best-effort, never fails a run. Frees the GPU between runs at the cost of a cold reload next run. +8 tests (worker 435). **Minor (deferred):** the gated Tailscale step runs on every ~15-min scheduled tick (due-ness is decided inside the job, not at the workflow level) → brief ephemeral `tag:ci` node churn on no-op ticks; harmless. **Status: ACCEPTED.**

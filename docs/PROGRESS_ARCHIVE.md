...
## Phase 39 "Current state" block (2026-06-01) — SUPERSEDED by Phase 40 DONE (ADR-144)

**Phase 39 DONE (AI-filter cost reduction; ADR-142 ACCEPTED + ADR-143).** Shipped, worker-only: the opt-in `cache_system` prompt-cache plumbing through `call_llm`→`_anthropic.call` (+ real cache-token fields on `LLMResponse`, cache-aware `pricing.estimate_cost_usd`, all three run-cost panels) and the output-trim (filter prompt omits `reason` on `pass:true`). **The caching premise (ADR-142) was measurement-falsified (ADR-143):** the filter system prompt is **1,861 tok** (heaviest committed profile, via `count_tokens`) — *below* Haiku 4.5's ~2048 cacheable minimum — so `cache_control` is a graceful **no-op** (2 real runs: `cw=0 cr=0`); per-batch input is dominated by the sent-once, uncacheable listing payload. Plumbing is correct (verified above the minimum with a synthetic system). **Owner call: keep the infra (harmless, future-proof) + document honestly** — real saving ≈ ~$0.004/run (output-trim), NOT ~40%. Determinism preserved (temp=0; byte-identical offline regression). **Verified: worker 380/380 (+15), ruff+mypy clean; web tsc/eslint clean, guards 70/70, parity 4/4, `next build` green.** Repro: `worker/scripts/filter_cache_livecheck.py`.

---

## Phase 34 STARTED block (2026-05-31) — SUPERSEDED by Phase 34 DONE (ADR-137)

- 2026-05-31 **Phase 34 STARTED — v2 onboarder FOUNDATION landed (verified, INERT; wiring deferred — see NEXT_SESSION_PHASE34.md).** Owner signed off scope at session start (2 `AskUserQuestion` forks): live preview = **model-invoked `serper_preview` tool** (results also stream to the user pane); **keep `web_search`** for product research; no vendor probing. Committed four NEW files that **nothing imports yet** (so the live onboarder + Vercel build are unaffected): **`web/lib/serper.ts`** (edge-safe TS Serper shopping client mirroring `adapters/serper.py` — `parseSerperPrice`/`mapSerperItem`/`mapSerperResponse`/`serperShoppingPreview`; never throws, returns `{ok,error}`); **`web/lib/onboard/validation-v2.ts`** (`validateProfileDraftV2` + `renderProfileV2Yaml` + `aliasIsDistinctive` — mirrors the parts of `profile_v2.py` the onboarder can botch: `queries` required, distinctive carry-gate aliases, real slug, ≥1 enabled source, edit-mode slug-pin; drops every v1 probe-era guard); **`web/lib/onboard/promptTextV2.ts`** (hand-written v2 conversational-intake prompt — no vendor curation/probing/`sources_pending`; documents the `serper_preview`/`web_search`/`validate_profile` tools + the `<state>`/`<draft>` protocol + v2 draft shape); **`web/scripts/check-onboard-v2.test.mjs`** (17 guard tests, wired into `test:guards`). **Verified GREEN:** `test:guards` **77/77** (60 v1 + 17 v2), `tsc --noEmit` clean, `eslint` clean. **`prompt.ts` was deliberately NOT switched to promptTextV2** — flipping it without the matching chat-route/save-route/UI changes would half-break the live onboarder. **NEXT SESSION = the atomic wiring** (prompt.ts → chat route → save route → OnboardChat). **Deploy prereq:** add `SERPER_API_KEY` to Vercel env. **Log ADR-137 once the full phase lands.** **Environment caveat → RESOLVED:** the OneDrive working tree's sync engine intermittently corrupted file-content reads, which is why the wiring was deferred. The repo was relocated to `C:\dev\product-search` (outside OneDrive) and the gitignored secrets were copied in.
...
## Current state — 2026-05-25 Subscription / non-RAM price-display correctness (ADR-094) (SUPERSEDED by 2026-05-26 Remaining onboarder paper-cuts + ADR-093/091 live re-verify / ADR-095)
...
## Current state — 2026-05-25 Run-now UX paper-cut + backend wall-budget tightening (ADR-093) (SUPERSEDED by 2026-05-25 Subscription / non-RAM price-display correctness / ADR-094)
...
## Current state — 2026-05-25 onboarder paper-cut — `description:` schema-vs-onboarder gap (ADR-092) (SUPERSEDED by 2026-05-25 Run-now UX paper-cut + backend wall-budget tightening / ADR-093)

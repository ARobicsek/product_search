...
- **Vendor quirks go in the registry, not a profile (ADR-068).** When you fix a vendor-level quirk — a URL transform (e.g. Best Buy `&intl=nosplash`), a default `alterlab_options` block, a known failure (e.g. microcenter Cloudflare), or AlterLab-known-good status — encode it in `worker/src/product_search/vendor_quirks.yaml` and regenerate the web artifacts (`node web/scripts/sync-prompt.js`). Then, if a specific profile also needs the change immediately, edit the profile too. **A per-profile patch *without* a matching registry update is a regression**: it fixes one product and silently lets every future onboarding of that vendor repeat the mistake — exactly the e93fd47 drift this registry exists to prevent. The registry is read by the runtime adapter, the onboarder prompt, and the save-time gate, so one edit propagates everywhere.
